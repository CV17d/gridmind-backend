...
public interface EnergyConsumptionRepository extends JpaRepository<EnergyConsumption, Long> {
    ...
    @Query(value = "SELECT ec.* FROM energy_consumptions ec JOIN devices d ON ec.device_id = d.id WHERE d.user_id = :userId AND ec.timestamp > NOW() - INTERVAL '1 hour' ORDER BY ec.timestamp DESC LIMIT 100", nativeQuery = true)
